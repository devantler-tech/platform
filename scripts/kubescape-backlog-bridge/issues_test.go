...
func TestScannerDerivedFieldsAreBoundedInTheBody(t *testing.T) {
	...
	for _, line := range strings.Split(body, "\n") {
		...
		case strings.HasPrefix(line, "**Severity:** "):
			// The code span's delimiters are markup, not the field; trimmed by
			// exact prefix and suffix for the reason given below.
			value := strings.TrimSuffix(strings.TrimPrefix(line, "**Severity:** `"), "`")
...
//
// The image case is the sharp one: any scanner-derived value interpolated
// outside a code span lets a scanner-controlled `![x](https://…)` make the issue
// page fetch that URL — a public security issue turned into a tracking beacon —
// and a wide image can push the affected-component list out of view.
...
// Severity is scanner-derived and sits in a live-Markdown line, so it is
// rendered inside a code span, the containment the component list already
// uses. A code span is escapable only by a backtick, which sanitizeForIssue maps
// to "'", so every metacharacter is inert without enumerating them one review
// round at a time (platform#2928).
func TestSeverityCannotRenderLiveMarkup(t *testing.T) {
	for _, severity := range []string{"_High_", "~~High~~", "a|b", "&amp;", "x`+live`y"} {
		th := theme{Kind: string(surfaceCVE), Key: "high", Severity: severity, Components: []string{"app/deployment/api"}, Count: 1, Total: 1}

		var line string
		for _, l := range strings.Split(renderBody(th), "\n") {
			if strings.HasPrefix(l, "**Severity:** ") {
				line = l
			}
		}

		value := strings.TrimPrefix(line, "**Severity:** ")
		if !strings.HasPrefix(value, "`") || !strings.HasSuffix(value, "`") || len(value) < 2 {
			t.Errorf("severity %q must render inside a code span, got %q", severity, line)

			continue
		}

		if inner := value[1 : len(value)-1]; strings.Contains(inner, "`") {
			t.Errorf("severity %q escapes its code span: %q", severity, line)
		}

		if first, second := renderBody(th), renderBody(th); first != second {
			t.Errorf("severity %q renders non-deterministically", severity)
		}
	}
}
