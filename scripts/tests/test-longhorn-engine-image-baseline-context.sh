...
# mutate-existing C-0211 retrofit for the default engine-image DaemonSet (#3918).

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd "${script_dir}/../.." && pwd)"
readonly script_dir repo_root
readonly policy="${repo_root}/k8s/providers/hetzner/infrastructure/cluster-policies/add-existing-longhorn-engine-image-baseline-context.yaml"
...
readonly fixtures_dir="${repo_root}/tests/longhorn-engine-image-baseline-context"
readonly grant_name="kyverno:background-controller:mutate-longhorn-engine-image"
readonly target='engine-image-ei-493e04e7'
...
grep -Fq 'cluster-policies/add-existing-longhorn-engine-image-baseline-context.yaml' "${infrastructure_kustomization}" ||
...
[[ "$(yq -r '.spec.rules[0].match.any[0].resources.names | join(",")' "${policy}")" == "${target}" ]] ||
  fail 'the rule must match only engine-image-ei-493e04e7'
...
[[ "$(yq -r '.spec.rules[0].mutate.targets[0] | .apiVersion + "|" + .kind + "|" + .namespace + "|" + .name' "${policy}")" == "apps/v1|DaemonSet|longhorn-system|${target}" ]] ||
  fail 'the rule must target exactly longhorn-system/engine-image-ei-493e04e7, statically named'
...
[[ "$(any "${live}" '(.spec.template.spec.containers[0].securityContext // {} | has("seLinuxOptions"))')" == 'true' ]] ||
  fail 'the engine-image container must receive seLinuxOptions'
[[ "$(any "${live}" '((.spec.template.spec.containers[0].securityContext.seLinuxOptions // {"x":1}) | length) == 0')" == 'true' ]] ||
  fail "the engine-image container's seLinuxOptions must stay empty — never pin a level or type"
[[ "$(any "${live}" '(.spec.template.spec.containers[0].securityContext | keys | sort | join(",")) == "privileged,seLinuxOptions"')" == 'true' ]] ||
  fail 'the privileged container must keep its existing securityContext — the object must never be replaced'
[[ "$(any "${live}" '(.spec.updateStrategy.rollingUpdate.maxUnavailable == "100%") and (.spec.selector.matchLabels["longhorn.io/engine-image"] == "ei-493e04e7")')" == 'true' ]] ||
  fail 'the retrofit must leave the manager-owned strategy and selector alone'

# Near misses stay untouched — another namespace, another engine version, and
# the csi-plugin DaemonSet that carries its own retrofit.
[[ "$(docs "${live}" tenant-control "${target}")" == '0' ]] ||
  fail 'the same DaemonSet name in another namespace must stay unchanged'
[[ "$(docs "${live}" longhorn-system engine-image-ei-a4d05f02)" == '0' ]] ||
  fail 'another engine version must stay unchanged — the CREATE-time rules own it'
[[ "$(docs "${live}" longhorn-system longhorn-csi-plugin)" == '0' ]] ||
  fail 'the csi-plugin DaemonSet must stay with its own retrofit'
...
# --- Grant: the Role stays exactly as narrow -----------------------------
...
[[ "$(role_query '(.rules[0].resourceNames // []) | join(",")')" == "${target}" ]] ||
  fail 'the mutation Role must stay pinned to engine-image-ei-493e04e7'
...
grep -Fq 'scripts/tests/test-longhorn-engine-image-baseline-context.sh' "${repo_root}/.github/workflows/ci.yaml" ||
  fail 'CI must execute the engine-image retrofit regression test'

completed=1
echo 'engine-image C-0211 retrofit is exact, additive, convergent, and uses a pinned grant.'
