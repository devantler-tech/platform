...
func TestDeployActionConsumerStagingPrecedesPublishAndIsReassertedAfterUpdate(t *testing.T) {
	...
	apiStability := requireIndex(t, action, "id: wait_prod_api_stability")
	...
	requireBefore(t, clusterUpdate, apiStability, "cluster update before API stability proof")
	requireBefore(t, apiStability, finalRefresh, "API stability proof before final refresh")
	requireContains(t, action[apiStability:finalRefresh], "run: ./scripts/wait-for-prod-api-stability.sh")
	requireContains(t, action[apiStability:finalRefresh], "steps.cluster_update.outcome != 'skipped'")
	...
	requireContains(t, finalRefreshStep, "steps.cluster_update.outcome == 'skipped'")
	requireContains(t, finalRefreshStep, "steps.wait_prod_api_stability.outcome == 'success'")
