...
// That alert established aggregate:
//
//	c69b141e14b9a2be136e57f8fb738e20207f35a378dfefdfdcaf3dfa1012e4ae
//
// Moved again by the C-0211 baseline context for the remaining cert-manager
// namespace workloads (#3239), derived on main 5599d0d9, whose approved
// aggregate is c69b141e above. origin-ca-issuer, simply-dns-webhook and
// trust-manager gain the same two inert fields, through the post-render patches
// for the first two and the chart's own values for trust-manager. A HelmRelease
// is a controller-RBAC emitter, so a pure VALUES change moves this aggregate
// even though nothing is granted.
//
// CONSERVATION: exactly three existing documents change: the cert-manager/
// origin-ca-issuer, cert-manager/simply-dns-webhook and cert-manager/
// trust-manager HelmReleases. Rendering k8s/providers/hetzner/infrastructure/
// controllers on this branch and on main 5599d0d9 yields 211 documents per side
// with an identical identity set, and the grant-bearing identity set
// (ClusterRole, Role, ClusterRoleBinding, RoleBinding, ServiceAccount) is
// identical at 46 per side. Injecting one synthetic ClusterRole into the branch
// render makes that comparison report it, so the identical result is a finding
// rather than a blind read. No identity, binding, ServiceAccount, verb,
// wildcard, AWS identity or permission changes; only those three HelmReleases'
// unresolved-substitution fingerprints move.
//
// RENDERER PROVENANCE: the value below was read from CI's own failure on job
// 103735999819 for head 3fa518d3 (the merge of main 5599d0d9 into this branch),
...
//
// Re-approved for #3760 (2026-09-13), derived on main f83314c9, whose approved
// aggregate is 0270e54e above. The change adds one pod annotation under
// reportsController in the kyverno HelmRelease values, so a clean restart of the
// reports controller is a recorded Git change. A HelmRelease is a
// controller-RBAC emitter, so a pure VALUES change moves this aggregate even
// though nothing is granted.
//
// CONSERVATION: exactly one existing document changes: the kyverno/kyverno
...
// or permission changes; only the kyverno HelmRelease's
// unresolved-substitution fingerprint moves.
//
// RENDERER PROVENANCE: the value below was read from CI's own failure on job
// 103776306632 for head c52d3873 (this branch on main f83314c9), which renders
...
//
// That availability remediation established aggregate:
//
//	d3748031f44c6c308831f3a5c4a7668ec88e314fe4ebbb6e6b09ec38388f6f02
//
// Moved again by the Kubescape liveness correction in #3894, derived on exact
// main 822c1591f5526b3ef20e71b3dfa8780dfaf86f48, whose approved aggregate is
// d3748031 above. The kubescape HelmRelease gains one post-renderer patch that
// raises only the kubescape Deployment liveness timeout from the Kubernetes
// default of one second to five seconds. A HelmRelease is a controller-RBAC
// emitter, so this moves the aggregate even though nothing is granted.
//
// CONSERVATION: the exact branch diff changes one authorization-surface
// document, the kubescape/kubescape HelmRelease, and CI's surface-delta
// diagnostic names that same single identity. No ClusterRole, Role,
// ClusterRoleBinding, RoleBinding or ServiceAccount source changes; no identity,
// binding, verb, wildcard, AWS identity or permission changes. The other three
// changed files are the Coroot suppression CronJob and focused shell contracts,
// which are outside the selected authorization overlays.
//
// RENDERER PROVENANCE: the value below was read from CI's failure on job
// 105884620327 for head 137d4aae (merge ref c1266d24 on CI run 35438190101),
// which renders under the approved SHA256-verified kubectl v1.36.2 after its
// approval-base check passed. The gate's moved-entry diagnostic named only the
// kubescape/kubescape HelmRelease. This host's renderer is unapproved, so no
// local digest is claimed.
//
...
//
// That SOPS-projection change established aggregate:
//
//	bd15655a621002ee2f1bef6f44683c8cdc89abc90e47e1626ae80279234c405b
//
// Moved by the C-0211 baseline context for the Hetzner cloud controller
// manager (#3239), whose base is bd15655a above. The
// kube-system/hcloud-cloud-controller-manager HelmRelease values gain
// fsGroupChangePolicy: OnRootMismatch on the pod security context and an
// empty seLinuxOptions object on the container. A HelmRelease is a
...
// helm.toolkit.fluxcd.io/v2 HelmRelease kube-system/hcloud-cloud-controller-manager.
// No ClusterRole, Role, ClusterRoleBinding, RoleBinding or ServiceAccount
// source changes; no identity, binding, verb, wildcard, AWS identity or
// permission changes.
//
// RENDERER PROVENANCE: the value below was printed by CI's checksum-verified
// kubectl v1.36.2 renderer (TestValidateAuthorizationAcceptsCommittedPolicy) on
// this change merged with main 8e3b5641a3acaca778d396edcdce26d65182947d, whose
...
//
// That CCM baseline context established aggregate:
//
//	c6e42dad361a132d416925d2b10206b5979d4425b6702efec8c0fe165f170639
//
// Moved by the C-0211 baseline context for Tetragon (#3239), whose base is
// c6e42dad above. The kube-system/tetragon HelmRelease values gain
// fsGroupChangePolicy: OnRootMismatch on the agent and operator pod security
// contexts and an empty seLinuxOptions object on the agent, export-stdout and
...
//
// Moved again by the Longhorn CSI sidecar C-0211 retrofit in #3946, re-derived
// on exact main 5b535db3f712b79ef26e4345e457bed0ee858f85. The new Role grants
// only get, patch, and update on apps/deployments, pinned by resourceNames to
// csi-attacher, csi-provisioner, csi-resizer and csi-snapshotter in
// longhorn-system, and the RoleBinding grants it only to the existing
// kyverno/kyverno-background-controller service account. It grants no creates,
// deletes, lists, watches, wildcard resources, wildcard verbs, secrets,
// workload exec, impersonation, AWS identity, or unrelated subject.
//
// CONSERVATION: the approval-base diagnostic names exactly the two added
// entries rbac.authorization.k8s.io/v1 Role and RoleBinding
// kyverno:background-controller:mutate-csi-sidecars in longhorn-system. No
// existing Role, ClusterRole, RoleBinding, ClusterRoleBinding, ServiceAccount,
// identity, subject, wildcard, AWS identity, or unrelated permission moved. The
// ClusterPolicy, its fixtures and the regression test are outside the selected
// authorization surface.
//
// RENDERER PROVENANCE: the required CI job's checksum-verified kubectl v1.36.2
// renderer (Kustomize v5.8.1), job 106161329925, measured the aggregate below
// and named only those two entries against the exact approval base. The local
// host has kubectl v1.36.1, so it was rejected by the validator and is not
// misrepresented as an independent renderer.
//
// Previous aggregate: 716b62f9ac8ad9a93eef6ac29fba1561928e6e4e00ff66a107fe7d4b504333ec.
//
