...
CI runs **static manifest validation** on PRs that touch k8s-related paths (`k8s/**`, `ksail*.yaml`, `.sops.yaml`, `talos*/**`, or `ci.yaml`) — the `validate` job in `.github/workflows/ci.yaml` runs `ksail workload validate` for both the local and prod overlays plus a Kubescape `ksail workload scan --framework nsa --compliance-threshold 85`. It is fast, needs no secrets (so it runs on fork PRs too), and starts no cluster. There is **no longer a full-cluster system test**: the local Docker cluster is a thin manual test-bed (see [Local Development Cluster](#local-development-cluster)), not a CI prod stand-in.
...
- **ksail is pinned to 7.65.0** (the Setup step), because 7.66.x parallelised the in-process Helm render and made it racy — `ksail workload validate` fails with varying YAML parse errors and the scan score swings run-to-run. 7.65.0 renders serially and both are deterministic. Race: [devantler-tech/ksail#5371](https://github.com/devantler-tech/ksail/issues/5371).
- **The threshold (85) is a regression floor, not the actual score.** The Kubescape compliance score is **environment-dependent**: the same ksail binary on the same manifests reports ≈**87.4%** on the Linux CI runner but ≈**94%** locally (macOS) — a gap that is *not* the render mode, the framework cache, or PR-merge content (all ruled out). So the gate sits a few points under the stable CI value to catch real regressions without flaking on that variance; **CI is the source of truth, and the score can't be reproduced exactly offline.** It is far from 100 because most residual findings are runtime-enforced (Kyverno/Cilium) or genuinely unfixable — notably **C-0002** (KubeVirt operator `pods/exec`, needed to manage VMs) — and the CLI can't consume the platform's `ClusterSecurityException` CRs ([ksail#5369](https://github.com/devantler-tech/ksail/issues/5369)). Ratchet up as genuine gaps are fixed; never lower it.
