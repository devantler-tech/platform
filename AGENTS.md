...
8. `scripts/run-ksail-prod-with-pull-auth.sh workload reconcile` triggers Flux with Git/SOPS pull auth. Both normal delivery and DR then wait until `infrastructure-controllers` has applied the exact newly-published digest and reports `Ready`; Cluster Autoscaler release and Talos mutation fail closed before that proof.
