...
8. `scripts/run-ksail-prod-with-pull-auth.sh workload reconcile` triggers Flux with Git/SOPS pull auth. Both normal delivery and DR prove that `infrastructure-controllers` observed the exact newly-published digest. While the temporary Cilium `OnDelete` gate is active, that revision proof deliberately omits `Ready`, which cannot complete until the operator steps the pods; otherwise the exact revision must also report `Ready` before Cluster Autoscaler release or Talos mutation.
9. After `cluster update`, the full bridge reasserts every pull path in case a partial update or older managed state was applied. DR applies the same Cilium rollout guard around publish and convergence; an active gate stops after exact-revision proof for the documented operator-stepped handoff instead of timing out on impossible pre-step readiness. DR also runs the bridge after an OpenBao raft restore because the snapshot may contain an older GHCR value.
...
- **`.github/actions/deploy-prod`** — the composite action both regular deploy paths call (stage/verify all GHCR pull consumers → push → cosign-sign → attest SBOM + SLSA provenance → revalidate published artifact → Flux reconcile → exact published-revision proof → Ready proof when not operator-gated → Talos `cluster update` → final reassert), so the merge-queue and manual deploys can never drift. DR uses the same exact-revision wait and rollout guard, and stops for the documented operator handoff when the temporary gate is active. The Cilium homogeneous-device `OnDelete` gate suspends Cluster Autoscaler before publish and skips `cluster update` until a reviewed completion or rollback artifact has applied, because KSail owns that Deployment and could otherwise reopen a scale-up race mid-rollout. Secrets are passed as inputs because composite actions cannot read `secrets`.
