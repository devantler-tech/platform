...
- **The `validate` job pins two ksail binaries** (the Setup step): **7.65.0** for `ksail workload validate` and **7.66.4** (as `ksail-scan`) for the scan with `--no-render`. 7.66.x parallelised the in-process Helm render and made it racy, so `validate` fails with varying YAML parse errors (7.65.0 renders serially → deterministic). For the scan, the *rendered* score is environment-dependent (**87% in CI vs 94% offline**, plus the same race), so `--no-render` scans only the raw authored manifests → deterministic and environment-independent (94.20%). `--no-render` only exists in 7.66.x. Race: [devantler-tech/ksail#5371](https://github.com/devantler-tech/ksail/issues/5371). Collapse to one binary once it's fixed.
