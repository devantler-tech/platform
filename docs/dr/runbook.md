...
> rebuild** (new Talos and Kubernetes PKI, so the stored credentials no longer
> authenticate) and must be refreshed, or the prod deploy pipeline cannot
> connect. See Scenario 9 below. Replacing a single control-plane node does
> **not** make them stale: the API address is a floating IP that outlives the
> node.
...
> After a rebuild the cluster has a new Talos and Kubernetes PKI, and possibly a
> new API address (see Scenario 9), so the `prod` environment's `KUBE_CONFIG` /
> `TALOS_CONFIG` secrets are now stale.
...
After a **full rebuild** (Scenario 4) the Talos and Kubernetes PKI change, so
both secrets are stale even when the API address is unchanged. Whether the
address changes depends on the floating IP: `ksail cluster create` reuses the
cluster's KSail-owned floating IP (`prod-floating-ip`) when it still exists in
the Hetzner project, and creates a new one, with a new address, when it does
not. `ksail cluster delete` releases that floating IP, so a rebuild after a
delete always gets a new address. The deploy selects the current floating IP on
every run either way, so the refresh below is needed for the new credentials,
not for the address. Symptom in CI: the
