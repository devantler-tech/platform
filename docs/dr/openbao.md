...
> resource, so PushSecrets are not involved for that secret. Only secrets
> originating from SOPS cluster variables are seeded via PushSecrets.
...
4. PushSecrets re-seed all secrets from SOPS variables. The Docker
   provider's platform CA key pair is auto-generated by cert-manager and
